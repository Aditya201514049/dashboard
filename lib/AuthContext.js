  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (authUser) => {
      ...
      try {
        ...
        // If user is authenticated and on a sign in/up page, redirect to dashboard
        else if (authUser && (pathname === '/signin' || pathname === '/signup')) {
          console.log('User is authenticated and on auth page, redirecting to dashboard');
          router.push('/dashboard');
        } 
        // User is authenticated on a permitted page
        else if (authUser) {
