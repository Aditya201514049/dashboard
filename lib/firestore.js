...
export const addSale = async (saleData, productId, shopId, userId) => {
  try {
    ...
    // Use the date from the form if provided, otherwise use current timestamp
    let createdAt;
    if (saleData.date) {
      // Convert date string to Date object at 12:00pm to avoid timezone issues
      const dateParts = saleData.date.split('-');
      const year = parseInt(dateParts[0]);
      const month = parseInt(dateParts[1]) - 1; // JavaScript months are 0-indexed
      const day = parseInt(dateParts[2]);
      createdAt = new Date(year, month, day, 12, 0, 0);
    } else {
      createdAt = serverTimestamp();
    }
